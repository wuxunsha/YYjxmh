<template>
  <div>
    <div class="content">
      <div class="platform-activity-content">
        <div class="platform-activity-right">
...
export default {
  ...
  created () {
    // 获取列表信息
    this.getActivityFindByPage()
  },
  methods: {
    // 获取列表
    getActivityFindByPage () {
      const data = {
        page: this.querInfo.page,
        size: this.querInfo.size
      }
      homeApi.activityFindByPage(data).then((res) => {
        if (res.status === 200) {
